#!/usr/bin/env python3
"""Compare MD→ALIKED vs MD→XFeat at shortlist N (default 50).

Scalable regime only: MegaDescriptor ranks the gallery; each local matcher
scores the top-N candidates (no all-pairs local matching).

XFeat: https://arxiv.org/abs/2404.19174

Example:
  .venv/bin/python scripts/compare_xfeat_models.py \\
    --features-dir /Users/abui/@code/tes/model/features \\
    --data-dir data \\
    --cache-dir /tmp/base_model_sims \\
    --shortlist-n 50 \\
    --out docs/results/base_model_comparison_xfeat_N50.csv
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "docs/results/base_model_comparison_xfeat_N50.csv",
    )
    return p.parse_args()
