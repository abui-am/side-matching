#!/usr/bin/env python3
"""Compare MD→ALIKED vs MD→DeDoDe at shortlist N (default 50).

No all-pairs local matching: MegaDescriptor ranks the gallery, then each local
matcher only scores the top-N candidates. That is the scalable regime.

Example:
  .venv/bin/python scripts/compare_dedode_models.py \\
    --features-dir /Users/abui/@code/tes/model/features \\
    --data-dir data \\
    --cache-dir /tmp/base_model_sims \\
    --shortlist-n 50 \\
    --out docs/results/base_model_comparison_dedode_N50.csv
"""
...
from sides_matching.dedode_matching import (  # noqa: E402
    DeDoDeMatcher,
    load_or_compute_dedode_shortlist_similarity,
)
...
def evaluate_dataset(
    *,
    ...
    for flip in (False, True):
        ...
        # ALIKED: reuse full cached matrix, then cascade to N (same scores as shortlist-only).
        ...
        dedode_short = load_or_compute_dedode_shortlist_similarity(
            name=name,
            data_root=root,
            paths=image_paths,
            flip_query=flip,
        ...
        methods: dict[str, Prediction] = {
            ...
            f"MD→DeDoDe N={shortlist_n}": Prediction(
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "docs/results/base_model_comparison_dedode_N50.csv",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    t0 = time.time()
    matcher = DeDoDeMatcher(device=args.device)
    print(f"DeDoDe L+B loaded in {time.time() - t0:.1f}s")
    ...
    print(f"\n=== SUMMARY (flip=True, N={args.shortlist_n}, full top-1) ===")
